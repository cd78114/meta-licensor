...
FullLicenseInfo::FullLicenseInfo(const string& _source, const string& _product,
		const string& _license_signature, int _license_version, string _from_date,
		string _to_date, const string& _client_signature,
		unsigned int _from_sw_version, unsigned int _to_sw_version,
		const string& _extra_data) :
		source(_source), product(_product), //
		license_signature(_license_signature), license_version(_license_version), //
		from_date(_from_date), to_date(_to_date), //
		has_expiry(_to_date!=UNUSED_TIME), //
		from_sw_version(_from_sw_version), to_sw_version(_to_sw_version), //
		has_versions(
				from_sw_version != UNUSED_SOFTWARE_VERSION
						|| to_sw_version != UNUSED_SOFTWARE_VERSION), //
		client_signature(_client_signature), has_client_sig(
...
LicenseReader::LicenseReader(const LicenseLocation& _licenseLocation) :
		licenseLocation(_licenseLocation) {
...
EventRegistry LicenseReader::getLicenseDiskFiles(vector<string>& diskFiles) {
	...
	if (licenseLocation.openFileNearModule) {
		...
		if (fret == FUNC_RET_OK) {
			...
			LOG_WARN("Error determining module name.",0);
		}
	}
...
void FullLicenseInfo::printAsIni(ostream & a_ostream) const {
	CSimpleIniA ini;
	string result;
	string _product = toupper_copy(trim_copy(this->product));
	CSimpleIniA::StreamWriter sw(a_ostream);
	ini.SetLongValue(_product.c_str(), "license_version", METALICENSOR_INT_VERSION);
	ini.SetValue(_product.c_str(), "license_signature", this->license_signature.c_str());
	if (has_client_sig) {
		ini.SetValue(_product.c_str(), "client_signature", this->client_signature.c_str());
	}
	if (has_versions) {
		ini.SetLongValue(_product.c_str(), "from_sw_version", from_sw_version);
		ini.SetLongValue(_product.c_str(), "to_sw_version", to_sw_version);
	}
	if (this->from_date != UNUSED_TIME) {
		ini.SetValue(_product.c_str(), "from_date", from_date.c_str());
	}
	if (this->to_date != UNUSED_TIME) {
		ini.SetValue(_product.c_str(), "to_date", to_date.c_str());
	}
	if (this->extra_data.length() > 0) {
		ini.SetValue(_product.c_str(), "extra_data", this->extra_data.c_str());
	}
	ini.Save(sw);
}
